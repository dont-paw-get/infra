...
- [ ] **Grafana 알림 규칙이 하나도 로드 안 되던 문제 발견·수정 (2026-08-29, Phase 2 진행 중)** — `monitoring/alerting/rules/*.yaml`에 `relativeTimeRange`가 없어 Grafana가 프로비저닝 reload를 통째로 거부(`invalidRelativeTime`, POST reload 500). contact point/policy만 로드되고 규칙 3종은 0개였다. 저장소 배포 이후 알림이 한 번도 동작하지 않았음. 5개 파일에 `relativeTimeRange: {from: 600, to: 0}` 추가(커밋/배포 대기). 경위는 `.harness/DECISIONS.md` 2026-08-29

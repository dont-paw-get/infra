...
- [x] app-level 알림 재배포 dev 실측 검증 완료 (2026-09-03, CLIAR-238, 브랜치 `CLIAR-238-app-level-알림-실측-반영`) — `grafana-alerting` ArgoCD app `Synced`/`Healthy`(ConfigMap 7종), Prometheus `Status > Targets`에서 `serviceMonitor/dpyb-<svc>-dev/backend-<svc>` **5개 전부 `UP`**, `http_server_requests_seconds_count`/`_bucket{application="backend-<svc>"}` 5개 서비스 조회 확인, Grafana provisioned alert-rule **6종 전부 `health: ok`/`state: inactive`**(`http-5xx-error-rate`/`http-p99-latency` 포함), Loki 스트림 라벨 `app`=`backend-<svc>` 5개 확인(파드에 `app.kubernetes.io/name` 존재 → `로그 ERROR 급증` `by (app)` 정상). 로그량도 정상 범위(auth 712·book 43·librarian 1.1k·record 1.2k·discovery 0 줄/h)
  - backend-book은 최초 확인 시 `serviceMonitor/dpyb-book-dev/backend-book` 타겟이 `server returned HTTP status 401`(관리 포트 `:8081/actuator/prometheus`에 Spring Security)로 `DOWN`이었으나, backend-book 저장소에서 무인증 허용으로 수정·dev 머지 → 2026-09-03 02:09 ArgoCD sync·02:15 파드 교체(이미지 `526e6b6f`) 후 `/actuator/prometheus` HTTP 200, 타겟 `UP` 전환 확인
  - threshold(5xx 5% / p99 1s)는 실트래픽이 거의 없어(5xx 샘플은 record `502` 소수) 튜닝 보류 — `.harness/PLAN.md` "알림 규칙 튜닝"

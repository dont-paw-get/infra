...
- Rules 5종 파일 존재(`monitoring/alerting/rules/`, 규칙 7개 — PVC는 warning/critical 2단계): HTTP 5xx 에러율, p99 레이턴시, CrashLoopBackOff/OOMKilled, PVC 사용률(80%/90%), 로그 ERROR 급증. threshold는 SLO 기준 1차 정리값(2026-09-03, `.harness/DECISIONS.md`) — 실트래픽 분포 확인 후 재조정 예정(`.harness/PLAN.md` "실측 후 재검증").
- 5종 **전부 `monitoring/alerting/kustomization.yaml`의 `configMapGenerator`에 등록되어 배포된다**(ConfigMap 7개 = 규칙 파일 5 + discord + notification-policy). HTTP 5xx 에러율/p99 레이턴시는 2026-08-25~09-02 동안 서비스 저장소 계측이 없어 제외돼 있었으나, `backend-*` 5개 서비스(auth/book/librarian/record/discovery)가 Micrometer(`http_server_requests_seconds_*`, `application` 라벨) 노출 + dev overlay `ServiceMonitor`를 추가하면서 재등록했다(CLIAR-238). 5개 모두 Micrometer 표준 메트릭 이름이라 규칙 쿼리(`by (application)`, `status=~"5.."`, `_bucket` + `le`)는 수정하지 않았다 — 아래 "서비스 저장소와의 경계" 표 참고.
...
- `http-p99-latency`/`http-5xx-error-rate` 규칙은 최소 트래픽 게이트를 갖는다: refId A 쿼리 뒤에 `and sum(rate(http_server_requests_seconds_count[5m])) by (application) >= 0.5`를 붙여 5분에 150건(0.5 req/s) 미만인 서비스는 평가에서 제외한다(CLIAR-261에서 0.2로 도입 — 2026-09-03 dev 실측에서 idle 서비스가 느린 요청 1건에 p99가 튀어 `Pending`으로 잡힌 문제 — SLO 재조정 시 표본 안정성을 위해 0.5로 상향). 두 규칙 모두 `noDataState: OK` — 게이트로 전 서비스가 빠져 refId A가 비면 `NoData`는 `DatasourceNoData` 알림을 내므로 "트래픽 없음 = 알림 없음"으로 처리한다.
- p99 레이턴시 규칙은 `application!~"backend-librarian|backend-discovery"`로 두 서비스를 제외한다 — Bedrock 호출 경로가 섞여 `application` 단위 p99가 상시 수 초이기 때문. 두 서비스의 일반 API 레이턴시는 URI 단위 SLO 규칙으로 별도로 다룬다(미구현, `.harness/PLAN.md`).
- 현재 threshold(SLO 기준 1차): 5xx 에러율 2% / p99 1s / PVC 80%(warning)·90%(critical) / 로그 ERROR 5분 10건. 게이트 0.5 req/s. 전부 실트래픽 확인 후 재조정 대상.
